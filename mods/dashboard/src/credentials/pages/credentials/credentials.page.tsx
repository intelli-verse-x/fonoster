...
export function meta(_: Route.MetaArgs) {
  return [
    { title: `Credentials | ${PRODUCT_NAME}` },
    {
      name: "description",
      content:
        "Credentials are a username and password that SIP agents and trunks use to sign in."
    }
  ];
}
...
const CredentialRow = styled(Box)(() => ({
  ...
  "&:hover .credential-open-icon": {
    color: "#fff",
    backgroundColor: "rgba(76,111,255,0.16)"
  },
  "&:active .credential-open-chevron": {
...
export default function CredentialsList() {
  ...
  const onOpen = useCallback(
    (ref: string) => {
      navigate(
        `/workspaces/${workspaceId}/sip-network/credentials/${ref}/edit`
      );
    },
      ...
      <Hero>
        <Box>
          ...
          <Typography sx={{ mt: 1, color: "base.04", fontSize: 14, maxWidth: 560 }}>
            A username and password that SIP phones and trunks use to sign in.
            Agents pick a credential when they register.
          </Typography>
        </Box>
        <Button
          variant="contained"
      ...
      <Board>
        <Toolbar>
          <Select
            ...
            sx={fieldSelectSx}
          >
            {CREDENTIALS_SEARCHABLE_FIELDS.map((field) => (
        ...
        {!isLoading && filteredData.length === 0 && (
          <Typography sx={{ px: 2, py: 4, color: "base.05", fontSize: 13 }}>
            No credentials yet. Create a username and password so an agent can
            sign in.
          </Typography>
        )}

        {!isLoading &&
          filteredData.map((credential: Credentials) => (
            <CredentialRow
              key={credential.ref}
              role="link"
              tabIndex={0}
              onClick={() => onOpen(credential.ref)}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
                  e.preventDefault();
                  onOpen(credential.ref);
                }
              }}
            >
                ...
                >
                  <Box>
                    ...
                    <Typography sx={{ fontSize: 12, color: "base.05" }}>
                      What the phone or trunk uses to sign in
                    </Typography>
                  </Box>
                </Box>
              </Box>
              ...
              <Box
                className="credential-open-icon"
                sx={{
                  mt: 0.5,
                  mr: 1,
                  width: 34,
              ...
              >
                <ChevronRightIcon
                  className="credential-open-chevron"
                  sx={{
                    fontSize: 20,
