# Graph Report - src  (2026-08-15)

## Corpus Check
- cluster-only mode — file stats not available

## Summary
- 13385 nodes · 26697 edges · 690 communities (383 shown, 307 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 1351 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e58d062a`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Community 0
- Community 1
- Community 2
- Community 3
- Community 4
- Community 5
- Community 6
- Community 7
- Community 8
- Community 9
- Community 10
- Community 11
- Community 12
- Community 13
- Community 14
- Community 15
- Community 16
- Community 17
- Community 18
- Community 19
- Community 21
- Community 22
- Community 23
- Community 24
- Community 25
- Community 26
- Community 27
- Community 28
- Community 29
- Community 30
- Community 31
- Community 32
- Community 33
- Community 34
- Community 35
- Community 36
- Community 37
- Community 38
- Community 39
- Community 40
- Community 41
- Community 42
- Community 43
- Community 44
- Community 45
- Community 46
- Community 47
- Community 48
- Community 49
- Community 50
- Community 51
- Community 52
- Community 53
- Community 54
- Community 55
- Community 56
- Community 57
- Community 58
- Community 59
- Community 60
- Community 61
- Community 62
- Community 63
- Community 64
- Community 65
- Community 66
- Community 67
- Community 68
- Community 69
- Community 70
- Community 71
- Community 72
- Community 73
- Community 74
- Community 75
- Community 76
- Community 77
- Community 78
- Community 79
- Community 80
- Community 81
- Community 82
- Community 83
- Community 84
- Community 85
- Community 86
- Community 87
- Community 88
- Community 89
- Community 90
- Community 91
- Community 92
- Community 93
- Community 94
- Community 95
- Community 96
- Community 97
- Community 98
- Community 99
- Community 100
- Community 101
- Community 102
- Community 103
- Community 104
- Community 105
- Community 106
- Community 107
- Community 108
- Community 109
- Community 110
- Community 111
- Community 113
- Community 114
- Community 115
- Community 116
- Community 117
- Community 118
- Community 119
- Community 120
- Community 121
- Community 122
- Community 123
- Community 124
- Community 125
- Community 126
- Community 127
- Community 128
- Community 129
- Community 130
- Community 131
- Community 132
- Community 133
- Community 134
- Community 135
- Community 136
- Community 137
- Community 138
- Community 139
- Community 140
- Community 141
- Community 142
- Community 143
- Community 144
- Community 145
- Community 146
- Community 147
- Community 148
- Community 149
- Community 150
- Community 151
- Community 152
- Community 153
- Community 154
- Community 155
- Community 156
- Community 157
- Community 158
- Community 159
- Community 160
- Community 161
- Community 162
- Community 163
- Community 164
- Community 172
- Community 173
- Community 174
- Community 175
- Community 176
- Community 177
- Community 178
- Community 179
- Community 180
- Community 181
- Community 182
- Community 183
- Community 184
- Community 185
- Community 186
- Community 187
- Community 188
- Community 189
- Community 190
- Community 191
- Community 192
- Community 193
- Community 194
- Community 195
- Community 196
- Community 197
- Community 198
- Community 199
- Community 200
- Community 201
- Community 202
- Community 203
- Community 204
- Community 205
- Community 206
- Community 207
- Community 208
- Community 209
- Community 210
- Community 212
- Community 213
- Community 214
- Community 215
- Community 216
- Community 217
- Community 218
- Community 219
- Community 220
- Community 221
- Community 222
- Community 223
- Community 224
- Community 225
- Community 226
- Community 227
- Community 228
- Community 229
- Community 230
- Community 231
- Community 232
- Community 233
- Community 234
- Community 235
- Community 236
- Community 237
- Community 238
- Community 239
- Community 240
- Community 241
- Community 242
- Community 243
- Community 244
- Community 245
- Community 246
- Community 247
- Community 248
- Community 249
- Community 250
- Community 251
- Community 252
- Community 254
- Community 255
- Community 256
- Community 257
- Community 258
- Community 259
- Community 260
- Community 261
- Community 262
- Community 263
- Community 264
- Community 265
- Community 266
- Community 267
- Community 268
- Community 269
- Community 270
- Community 271
- Community 272
- Community 273
- Community 274
- Community 275
- Community 276
- Community 277
- Community 278
- Community 279
- Community 280
- Community 281
- Community 282
- Community 283
- Community 284
- Community 285
- Community 286
- Community 287
- Community 288
- Community 289
- Community 290
- Community 291
- Community 292
- Community 293
- Community 294
- Community 295
- Community 296
- Community 297
- Community 298
- Community 299
- Community 300
- Community 301
- Community 302
- Community 303
- Community 304
- Community 305
- Community 306
- Community 307
- Community 308
- Community 309
- Community 310
- Community 311
- Community 312
- Community 313
- Community 314
- Community 315
- Community 316
- Community 317
- Community 318
- Community 319
- Community 320
- Community 321
- Community 322
- Community 323
- Community 324
- Community 325
- Community 326
- Community 327
- Community 328
- Community 329
- Community 330
- Community 331
- Community 332
- Community 333
- Community 334
- Community 335
- Community 336
- Community 337
- Community 338
- Community 339
- Community 340
- Community 341
- Community 342
- Community 343
- Community 344
- Community 345
- Community 346
- Community 347
- Community 348
- Community 349
- Community 350
- Community 351
- Community 352
- Community 353
- Community 354
- Community 355
- Community 356
- Community 357
- Community 358
- Community 359
- Community 360
- Community 361
- Community 362
- Community 363
- Community 364
- Community 365
- Community 366
- Community 367
- Community 368
- Community 369
- Community 370
- Community 371
- Community 372
- Community 373
- Community 374
- Community 375
- Community 376
- Community 377
- Community 378
- Community 379
- Community 380
- Community 381
- Community 382
- Community 383
- Community 384
- Community 385
- Community 386
- Community 387
- Community 388
- Community 389
- Community 390
- Community 391
- Community 392
- Community 393
- Community 394
- Community 395
- Community 396
- Community 397
- Community 398
- Community 399
- Community 400
- Community 401
- Community 402
- Community 403
- Community 404
- Community 405
- Community 406
- Community 407
- Community 408
- Community 409
- Community 410
- Community 411
- Community 412
- Community 413
- Community 414
- Community 415
- Community 416
- Community 417
- Community 418
- Community 419
- Community 420
- Community 421
- Community 422
- Community 423
- Community 424
- Community 425
- Community 426
- Community 427
- Community 428
- Community 429
- Community 430
- Community 431
- Community 432
- Community 433
- Community 434
- Community 435
- Community 436
- Community 437
- Community 438
- Community 439
- Community 440
- Community 441
- Community 442
- Community 443
- Community 444
- Community 445
- Community 446
- Community 447
- Community 448
- Community 449
- Community 450
- Community 451
- Community 452
- Community 453
- Community 454
- Community 455
- Community 456
- Community 457
- Community 458
- Community 459
- Community 460
- Community 461
- Community 462
- Community 463
- Community 464
- Community 465
- Community 466
- Community 467
- Community 468
- Community 469
- Community 470
- Community 471
- Community 472
- Community 473
- Community 474
- Community 475
- Community 476
- Community 477
- Community 478
- Community 479
- Community 480
- Community 481
- Community 482
- Community 483
- Community 484
- Community 485
- Community 486
- Community 487
- Community 488
- Community 489
- Community 490
- Community 491
- Community 492
- Community 493
- Community 494
- Community 495
- Community 496
- Community 497
- Community 498
- Community 499
- Community 500
- Community 501
- Community 502
- Community 503
- Community 504
- Community 505
- Community 506
- Community 507
- Community 508
- Community 509
- Community 510
- Community 511
- Community 512
- Community 513
- Community 514
- Community 515
- Community 516
- Community 517
- Community 518
- Community 519
- Community 520
- Community 521
- Community 522
- Community 523
- Community 524
- Community 526
- Community 527
- Community 528
- Community 529
- Community 530
- Community 531
- Community 532
- Community 533
- Community 534
- Community 535
- Community 536
- Community 537
- Community 538
- Community 539
- Community 540
- Community 542
- Community 543
- Community 544
- Community 545
- Community 546
- Community 547
- Community 548
- Community 549
- Community 550
- Community 551
- Community 552
- Community 553
- Community 554
- Community 555
- Community 556
- Community 557
- Community 558
- Community 559
- Community 560
- Community 561
- Community 562
- Community 563
- Community 564
- Community 565
- Community 566
- Community 567
- Community 568
- Community 569
- Community 570
- Community 571
- Community 572
- Community 573
- Community 574
- Community 575
- Community 576
- Community 577
- Community 578
- Community 579
- Community 580
- Community 581
- Community 582
- Community 583
- Community 584
- Community 585
- Community 586
- Community 587
- Community 588
- Community 589
- Community 590
- Community 591
- Community 593
- Community 594
- Community 595
- Community 596
- Community 597
- Community 598
- Community 599
- Community 600
- Community 601
- Community 602
- Community 603
- Community 604
- Community 605
- Community 606
- Community 607
- Community 608
- Community 609
- Community 610
- Community 611
- Community 612
- Community 613
- Community 614
- Community 615
- Community 616
- Community 617
- Community 618
- Community 619
- Community 620
- Community 621
- Community 622
- Community 623
- Community 624
- Community 625
- Community 626
- Community 627
- Community 628
- Community 629
- Community 630
- Community 631
- Community 632
- Community 633
- Community 634
- Community 635
- Community 636
- Community 638
- Community 639
- Community 640
- Community 641
- Community 642
- Community 643
- Community 644
- Community 645
- Community 646
- Community 647
- Community 648
- Community 649
- Community 650
- Community 651
- Community 652
- Community 653
- Community 654
- Community 655
- Community 656
- Community 657
- Community 658
- Community 659
- Community 660
- Community 661

## God Nodes (most connected - your core abstractions)
1. `useWorkspaceId()` - 147 edges
2. `getLogger()` - 139 edges
3. `Decimal` - 117 edges
4. `Decimal` - 117 edges
5. `n()` - 110 edges
6. `AuthenticatedCommand` - 101 edges
7. `useFonoster()` - 98 edges
8. `withErrorHandlingAndValidation()` - 93 edges
9. `o()` - 93 edges
10. `toast()` - 86 edges

## Surprising Connections (you probably didn't know these)
- `j()` --indirect_call--> `E()`  [INFERRED]
  site/site-assets/js/bootstrap.min.js → mods/identity/src/generated/@prisma/client/query_engine_bg.js
- `J()` --indirect_call--> `E()`  [INFERRED]
  site/site-assets/js/jquery.min.js → mods/identity/src/generated/@prisma/client/query_engine_bg.js
- `c()` --indirect_call--> `E()`  [INFERRED]
  site/site-assets/js/swiper-bundle.min.js → mods/identity/src/generated/@prisma/client/query_engine_bg.js
- `t()` --indirect_call--> `E()`  [INFERRED]
  site/site-assets/js/swiper-bundle.min.js → mods/identity/src/generated/@prisma/client/query_engine_bg.js
- `st()` --indirect_call--> `Q()`  [INFERRED]
  mods/identity/src/generated/@prisma/client/query_engine_bg.js → site/site-assets/js/jquery.min.js

## Import Cycles
- 3-file cycle: `mods/common/src/voice/Stream.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/Stream.ts`
- 3-file cycle: `mods/common/src/voice/PlayDtmf.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/PlayDtmf.ts`
- 3-file cycle: `mods/common/src/voice/Dial.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/Dial.ts`
- 3-file cycle: `mods/common/src/voice/Gather.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/Gather.ts`
- 3-file cycle: `mods/common/src/voice/Mute.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/Mute.ts`
- 3-file cycle: `mods/common/src/voice/Play.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/Play.ts`
- 3-file cycle: `mods/common/src/voice/Record.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/Record.ts`
- 3-file cycle: `mods/common/src/voice/Say.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/Say.ts`
- 3-file cycle: `mods/common/src/voice/StopSay.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/StopSay.ts`
- 3-file cycle: `mods/common/src/voice/StreamGather.ts -> mods/common/src/voice/Verb.ts -> mods/common/src/voice/voice.ts -> mods/common/src/voice/StreamGather.ts`
- 3-file cycle: `mods/common/src/identity/createAuthInterceptor.ts -> mods/common/src/identity/tokenHasAccessKeyId.ts -> mods/common/src/identity/index.ts -> mods/common/src/identity/createAuthInterceptor.ts`
- 4-file cycle: `mods/identity/src/exchanges/createExchangeCredentials.ts -> mods/identity/src/verification/index.ts -> mods/identity/src/verification/createSendVerificationCode.ts -> mods/identity/src/exchanges/index.ts -> mods/identity/src/exchanges/createExchangeCredentials.ts`
- 5-file cycle: `mods/autopilot/src/index.ts -> mods/autopilot/src/models/index.ts -> mods/autopilot/src/models/evaluations/index.ts -> mods/autopilot/src/models/evaluations/runEval.ts -> mods/autopilot/src/models/createLanguageModel.ts -> mods/autopilot/src/index.ts`
- 5-file cycle: `mods/autopilot/src/models/AbstractLanguageModel.ts -> mods/autopilot/src/models/createPromptTemplate.ts -> mods/autopilot/src/models/createSystemPrompt.ts -> mods/autopilot/src/models/index.ts -> mods/autopilot/src/models/LanguageModelFactory.ts -> mods/autopilot/src/models/AbstractLanguageModel.ts`
- 5-file cycle: `mods/autopilot/src/index.ts -> mods/autopilot/src/models/index.ts -> mods/autopilot/src/models/evaluations/index.ts -> mods/autopilot/src/models/evaluations/evalTestCases.ts -> mods/autopilot/src/models/createLanguageModel.ts -> mods/autopilot/src/index.ts`

## Communities (690 total, 307 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.00
Nodes (553): AggregateApiKey, AggregateUser, AggregateVerificationCode, AggregateWorkspace, AggregateWorkspaceMember, ApiKey, ApiKeyAggregateArgs, ApiKeyCountAggregateInputType (+545 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (343): AccelerateEngineConfig, AccelerateExtensionFetch, AccelerateExtensionFetchDecorator, AccelerateUtils, Action, ActiveConnectorType, Aggregate, AllModelsToStringIndex (+335 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (77): AudioSocketHandler, logger, AuthorizationHandler, ExternalMediaHandler, logger, GrpcClientHandler, logger, logger (+69 more)

### Community 3 - "Community 3"
Cohesion: 0.05
Nodes (105): logger, logger, logger, prisma, logger, listSecrets(), logger, logger (+97 more)

### Community 4 - "Community 4"
Cohesion: 0.02
Nodes (78): An(), applyPendingMigrations(), bo(), bs(), buildQueryError(), connect(), constructor(), consumeError() (+70 more)

### Community 5 - "Community 5"
Cohesion: 0.04
Nodes (103): CreateAclFormProps, CreateRuleModal(), ModalProps, Schema, CreateAgentFormProps, CreateApiKeyForm(), CreateApiKeyFormProps, Schema (+95 more)

### Community 6 - "Community 6"
Cohesion: 0.02
Nodes (102): ACLS_SEARCHABLE_FIELDS, AclRow, AclWithDeny, Board, FieldLabel, fieldSelectSx, Hero, previewCidrs() (+94 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (76): ac(), Ad(), Af(), Ai(), am(), Bd(), bi(), cc() (+68 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (69): addItem(), al(), ao(), bn(), bs(), bu(), Ci(), constructor() (+61 more)

### Community 9 - "Community 9"
Cohesion: 0.05
Nodes (108): AclsList(), EditAcl(), RESOURCE_QUERY_KEY, useAcl(), useAcls(), useCreateAcl(), useDeleteAcl(), useUpdateAcl() (+100 more)

### Community 10 - "Community 10"
Cohesion: 0.06
Nodes (62): CreateAclForm(), useCreateAcl(), CreateAcl(), Schema, COLLECTION_QUERY_KEY, ModalProps, useCreateAgent(), CreateAgent() (+54 more)

### Community 11 - "Community 11"
Cohesion: 0.02
Nodes (71): Af(), Bc(), bi(), Bm(), Bu(), ca(), Cm(), Co() (+63 more)

### Community 12 - "Community 12"
Cohesion: 0.03
Nodes (75): AuthenticationFlowHeader, HeaderRoot, BrandPanel, FormCard, FormPanel, LayoutContent, LayoutRoot, loader() (+67 more)

### Community 13 - "Community 13"
Cohesion: 0.02
Nodes (61): At(), bo(), cs(), dc(), dispatchEngineSpans(), du(), Ee(), es() (+53 more)

### Community 14 - "Community 14"
Cohesion: 0.03
Nodes (72): al(), ao(), ar(), bo(), bs(), buildQueryError(), constructor(), consumeError() (+64 more)

### Community 15 - "Community 15"
Cohesion: 0.03
Nodes (96): logger, client, influxdb, CallManagerConfig, createCreateCallSubscriber(), logger, allowList, httpBridge() (+88 more)

### Community 18 - "Community 18"
Cohesion: 0.03
Nodes (55): aa(), an(), as(), L(), cn(), ct(), dispatchEngineSpans(), dn() (+47 more)

### Community 19 - "Community 19"
Cohesion: 0.05
Nodes (51): AccessToken, RefreshToken, TokenUseEnum, SIGN_ALGORITHM, VERIFICATION_CODE_EXPIRATION, createPrismaClient(), Prisma, createExchangeApiKey() (+43 more)

### Community 21 - "Community 21"
Cohesion: 0.04
Nodes (69): AclsPageHeader(), AgentsPageHeader(), ApplicationsPageHeader(), ICON, Icon(), IconProps, Button(), DisabledButton (+61 more)

### Community 22 - "Community 22"
Cohesion: 0.05
Nodes (57): buildService(), createCreateApplication(), logger, createCreateTestToken(), logger, createDeleteApplication(), logger, createGetApplication() (+49 more)

### Community 23 - "Community 23"
Cohesion: 0.06
Nodes (66): buildWelcomeDemoService(), logger, SERVICE_DEFINITION, aclsService, agentsService, applicationsService, callsService, credentialsService (+58 more)

### Community 24 - "Community 24"
Cohesion: 0.09
Nodes (50): a(), addEventListener(), ae(), b(), blur(), c(), S(), cancelAnimationFrame() (+42 more)

### Community 25 - "Community 25"
Cohesion: 0.04
Nodes (58): AgreeTermsModal(), AgreeTermsModalProps, ActionsRoot, ForgotPasswordForm(), ForgotPasswordFormProps, Form, resolver, Schema (+50 more)

### Community 26 - "Community 26"
Cohesion: 0.05
Nodes (58): useSipTestCall(), useApplicationTestCall(), APPLICATION_TYPES, APPLICATIONS_DEFAULT_INITIAL_VALUES, getLanguageModelModels(), LANGUAGE_MODEL_ANTHROPIC_MODELS, LANGUAGE_MODEL_GOOGLE_MODELS, LANGUAGE_MODEL_GROQ_MODELS (+50 more)

### Community 27 - "Community 27"
Cohesion: 0.03
Nodes (27): AuthenticatedCommand, Create, Delete, List, Regenerate, Get, List, Delete (+19 more)

### Community 28 - "Community 28"
Cohesion: 0.06
Nodes (31): textChunksByFirstNaturalPause(), AbstractTextToSpeech, logger, Azure, Deepgram, ElevenLabs, logger, TODO: Make this configurable (+23 more)

### Community 29 - "Community 29"
Cohesion: 0.08
Nodes (35): AbstractLanguageModel, logger, Anthropic, AnthropicModel, AnthropicParams, createChatHistory(), createChain(), createPromptTemplate() (+27 more)

### Community 30 - "Community 30"
Cohesion: 0.05
Nodes (49): useAuth(), useAuthContext(), useAuthenticatedClient(), useCurrentUser(), StepProgress(), StepProgressProps, Form, resolver (+41 more)

### Community 31 - "Community 31"
Cohesion: 0.06
Nodes (34): ApiKeys, Domains, Users, Workspaces, CreateDomainRequest, ListDomainsRequest, ListDomainsResponse, UpdateDomainRequest (+26 more)

### Community 32 - "Community 32"
Cohesion: 0.05
Nodes (41): FonosterService, loadServices(), logger, handleError(), logger, handleZodError(), logger, PrismaErrorEnum (+33 more)

### Community 33 - "Community 33"
Cohesion: 0.06
Nodes (40): loadAndValidateAssistant(), logger, AssistantConfig, ConversationSettings, LanguageModelConfig, APISERVER_ENDPOINT, AWS_S3_ACCESS_KEY_ID, AWS_S3_ENDPOINT (+32 more)

### Community 34 - "Community 34"
Cohesion: 0.08
Nodes (38): createLanguageModel(), createEvalEffectiveConfig(), createEvaluateIntelligence(), logger, ServerStreamCall, createTestTextSimilarity(), evalTestCases(), evaluateScenario() (+30 more)

### Community 35 - "Community 35"
Cohesion: 0.07
Nodes (35): compileTemplate(), CompileTemplateParams, createSendEmail(), logger, createSendSmsTwilioImpl(), logger, EmailParams, EmailSenderConfig (+27 more)

### Community 36 - "Community 36"
Cohesion: 0.06
Nodes (26): Messages, toCamelCase(), toPascalCase(), PlaybackControlRequest, RecordOptions, RecordResponse, StartStreamGatherRequest, Answer (+18 more)

### Community 37 - "Community 37"
Cohesion: 0.03
Nodes (58): Commands, `fonoster apikeys:create`, `fonoster apikeys:delete REF`, `fonoster apikeys:list`, `fonoster apikeys:regenerate REF`, `fonoster applications:create`, `fonoster applications:delete REF`, `fonoster applications:eval` (+50 more)

### Community 38 - "Community 38"
Cohesion: 0.08
Nodes (57): dp(), nf(), Cl(), _a(), Ae(), Au(), byteLength(), #c() (+49 more)

### Community 39 - "Community 39"
Cohesion: 0.05
Nodes (53): ae(), ai(), Ba(), be(), br(), co(), Do(), _e() (+45 more)

### Community 40 - "Community 40"
Cohesion: 0.11
Nodes (55): aa(), addErrorMessage(), addField(), addItem(), addSuggestion(), ai(), asObject(), ba() (+47 more)

### Community 41 - "Community 41"
Cohesion: 0.11
Nodes (55): addErrorMessage(), addItem(), addSuggestion(), al(), asObject(), bl(), cl(), concat() (+47 more)

### Community 42 - "Community 42"
Cohesion: 0.05
Nodes (54): bn(), Ci(), constructor(), e(), ed(), F(), fn(), fp() (+46 more)

### Community 43 - "Community 43"
Cohesion: 0.11
Nodes (53): addErrorMessage(), addField(), addSuggestion(), ap(), ar(), asObject(), bp(), Cp() (+45 more)

### Community 44 - "Community 44"
Cohesion: 0.06
Nodes (19): AccessClaims, ApiKey, ApiKeyCredentials, ContactType, CreateApiKeyRequest, CreateUserRequest, ExchangeCredentialsRequest, ExchangeResponse (+11 more)

### Community 45 - "Community 45"
Cohesion: 0.04
Nodes (51): chai-as-promised, cross-env, eslint, eslint-config-next, eslint-plugin-import, eslint-plugin-react, eslint-plugin-react-hooks, globals (+43 more)

### Community 46 - "Community 46"
Cohesion: 0.07
Nodes (35): buildService(), createCall(), createCallPublisher(), createFetchCalls(), logger, createFetchSingleCall(), logger, createTrackCall() (+27 more)

### Community 47 - "Community 47"
Cohesion: 0.08
Nodes (17): countryIsoCodes, Create, Update, Create, Update, Create, Update, Create (+9 more)

### Community 48 - "Community 48"
Cohesion: 0.12
Nodes (50): ac(), addErrorMessage(), addField(), addSuggestion(), asObject(), bc(), cc(), dc() (+42 more)

### Community 49 - "Community 49"
Cohesion: 0.07
Nodes (19): a(), an(), bi(), de(), Ea(), Ei(), fi(), hc() (+11 more)

### Community 50 - "Community 50"
Cohesion: 0.04
Nodes (48): author, bin, fonoster, bugs, url, dependencies, deepmerge, @fonoster/common (+40 more)

### Community 51 - "Community 51"
Cohesion: 0.06
Nodes (48): bc(), bf(), commitTransaction(), constructor(), convertProtocolErrorsToClientError(), ct(), disconnect(), dispatchBatches() (+40 more)

### Community 52 - "Community 52"
Cohesion: 0.05
Nodes (5): AclsClient, AgentsClient, ApplicationsClient, CredentialsServiceClient, NumbersClient

### Community 53 - "Community 53"
Cohesion: 0.08
Nodes (13): AudioPlayer, AudioSocket, logger, AudioSocketError, Message, nextMessage(), ErrorCode, EventType (+5 more)

### Community 54 - "Community 54"
Cohesion: 0.05
Nodes (19): ./query_engine_bg.wasm?module, ae(), E(), et(), fe(), ft(), g(), ge() (+11 more)

### Community 55 - "Community 55"
Cohesion: 0.13
Nodes (46): addErrorMessage(), addField(), addSuggestion(), ap(), asObject(), at(), Do(), ec() (+38 more)

### Community 56 - "Community 56"
Cohesion: 0.07
Nodes (42): ba(), cn(), Da(), enabled(), Er(), eu(), Fa(), Fl() (+34 more)

### Community 57 - "Community 57"
Cohesion: 0.07
Nodes (45): $l(), addField(), au(), Bi(), ds(), e(), el(), eo() (+37 more)

### Community 58 - "Community 58"
Cohesion: 0.11
Nodes (19): Configure, LinkTwilioNumber, Active, List, Logout, Use, addWorkspace(), getActiveWorkspace() (+11 more)

### Community 59 - "Community 59"
Cohesion: 0.14
Nodes (44): ad(), addErrorMessage(), addField(), addItem(), addSuggestion(), asObject(), bd(), cd() (+36 more)

### Community 61 - "Community 61"
Cohesion: 0.13
Nodes (43): addErrorMessage(), addField(), addSuggestion(), l(), asObject(), bu(), Do(), fu() (+35 more)

### Community 62 - "Community 62"
Cohesion: 0.08
Nodes (16): a(), ar(), bi(), fromContent(), gl(), hn(), Je(), K() (+8 more)

### Community 63 - "Community 63"
Cohesion: 0.05
Nodes (42): author, bin, fonoster, bugs, url, dependencies, @fonoster/common, @fonoster/identity (+34 more)

### Community 64 - "Community 64"
Cohesion: 0.07
Nodes (4): q, remove(), W, we

### Community 65 - "Community 65"
Cohesion: 0.11
Nodes (36): Ae(), B(), Be(), c(), $e(), ee(), F(), fe() (+28 more)

### Community 66 - "Community 66"
Cohesion: 0.09
Nodes (17): assistantWithoutApiKeySchema, speechValidators, MethodNotImplementedError, AbstractSpeechToText, buildTranscribeConfig(), Deepgram, {
  DeepgramClient,
  LiveTranscriptionEvents,
  createClient
  // eslint-disable-next-line @typescript-eslint/no-var-requires
}, logger (+9 more)

### Community 67 - "Community 67"
Cohesion: 0.07
Nodes (10): VoiceImpl, DialOptions, MuteOptions, StreamOptions, StreamGatherOptions, StreamGatherPayload, VoiceRequest, VoiceSessionStreamServer (+2 more)

### Community 68 - "Community 68"
Cohesion: 0.08
Nodes (26): announceIdleTimeout(), logger, announceSystemError(), logger, appendSpeech, logger, cleanSpeech, goodbye() (+18 more)

### Community 69 - "Community 69"
Cohesion: 0.08
Nodes (42): Aa(), bf(), buildQueryError(), consumeError(), convertProtocolErrorsToClientError(), df(), dispatchBatches(), dr() (+34 more)

### Community 70 - "Community 70"
Cohesion: 0.10
Nodes (31): AclExtended, CreateAclRequestExtended, ListAclsResponseExtended, AgentExtended, AgentsApi, CreateAgentRequestExtended, ListAgentsResponseExtended, EvalErrorEvent (+23 more)

### Community 71 - "Community 71"
Cohesion: 0.08
Nodes (21): APISERVER_HOST, mapCallDirectionToEnum(), transformEvent(), createCreateVoiceClient(), generateCallAccessToken, logger, ApplicationNotFoundError, createCreateContainer() (+13 more)

### Community 72 - "Community 72"
Cohesion: 0.08
Nodes (39): _e(), bn(), _c(), dr(), es(), g(), getAllBatchQueryCallbacks(), getArgumentName() (+31 more)

### Community 73 - "Community 73"
Cohesion: 0.09
Nodes (41): ar(), Ea(), em(), eo(), Et(), findField(), ga(), getArgumentName() (+33 more)

### Community 74 - "Community 74"
Cohesion: 0.05
Nodes (41): scripts, build, bump:version, check:updates, clean, copy:hbstemplates, copy:prisma-clients, db:migrate (+33 more)

### Community 75 - "Community 75"
Cohesion: 0.05
Nodes (39): author, bugs, url, dependencies, deepmerge, @fonoster/common, @fonoster/identity, @fonoster/logger (+31 more)

### Community 76 - "Community 76"
Cohesion: 0.08
Nodes (22): se(), b(), be(), bn(), constructor(), F(), Fn(), _getName() (+14 more)

### Community 77 - "Community 77"
Cohesion: 0.08
Nodes (13): a(), Ai(), Ga(), l(), Le(), re(), Ri(), $s() (+5 more)

### Community 78 - "Community 78"
Cohesion: 0.05
Nodes (38): @modelcontextprotocol/sdk, author, bin, fonoster-mcp, bugs, url, dependencies, @fonoster/common (+30 more)

### Community 79 - "Community 79"
Cohesion: 0.10
Nodes (18): Autopilot, logger, AutopilotParams, chunkToFloat32Array(), createVad(), logger, logger, SileroVad (+10 more)

### Community 80 - "Community 80"
Cohesion: 0.08
Nodes (39): ai(), cp(), ec(), getOrCreate(), Hd(), hr(), ic(), io() (+31 more)

### Community 81 - "Community 81"
Cohesion: 0.05
Nodes (37): @chromatic-com/storybook, devDependencies, @chromatic-com/storybook, @react-router/dev, react-router-devtools, storybook, @storybook/addon-docs, @storybook/addon-essentials (+29 more)

### Community 82 - "Community 82"
Cohesion: 0.11
Nodes (21): ASTERISK_SYSTEM_DOMAIN, ASTERISK_TRUNK, CALLS_TRACK_CALL_SUBJECT, createHandleDialEventsWithNats(), createHandleDialEventsWithVoiceClient(), FailedStatus, mapDialStatus(), createDialHandler() (+13 more)

### Community 83 - "Community 83"
Cohesion: 0.08
Nodes (33): bi(), cc(), cn(), Da(), de(), ea(), Ei(), es() (+25 more)

### Community 84 - "Community 84"
Cohesion: 0.09
Nodes (37): wu(), Be(), buildQueryError(), consumeError(), convertProtocolErrorsToClientError(), Cu(), deref(), dispatchBatches() (+29 more)

### Community 85 - "Community 85"
Cohesion: 0.09
Nodes (20): EventsHook, logger, sendConversationEndedEvent(), FIXME: Workaround for Google LLMs issue, FIXME: Workaround for Google LLMs issue, transferToolDefinition, convertToolToLangchainTool(), LangchainToolParams (+12 more)

### Community 86 - "Community 86"
Cohesion: 0.06
Nodes (35): author, bugs, url, dependencies, @fonoster/logger, uuid, description, devDependencies (+27 more)

### Community 87 - "Community 87"
Cohesion: 0.06
Nodes (35): @emotion/cache, @emotion/react, @emotion/server, @emotion/styled, export-to-csv, @hookform/resolvers, dependencies, @emotion/cache (+27 more)

### Community 88 - "Community 88"
Cohesion: 0.14
Nodes (18): GatherOptions, GatherRequest, GatherResponse, MuteRequest, PlayOptions, PlayRequest, PlayResponse, PlaybackControlAction (+10 more)

### Community 89 - "Community 89"
Cohesion: 0.12
Nodes (12): AuthzClient, AuthzServiceClient, AuthzServer, logger, defaultServerConfig, DummyAuthzHandler, logger, serviceDefinition (+4 more)

### Community 90 - "Community 90"
Cohesion: 0.13
Nodes (22): createCheckMethodAuthorized(), logger, createAuthInterceptor(), logger, decodeToken(), permissionDeniedError(), unauthenticatedError(), hasAccess() (+14 more)

### Community 91 - "Community 91"
Cohesion: 0.09
Nodes (18): FormFieldContext, FormFieldContextValue, FormItemContext, FormItemContextValue, FormItemRoot, Toaster, ThemeToggle(), ThemeSwitch() (+10 more)

### Community 92 - "Community 92"
Cohesion: 0.09
Nodes (31): bo(), br(), co(), di(), enabled(), Er(), _getName(), gl() (+23 more)

### Community 93 - "Community 93"
Cohesion: 0.08
Nodes (30): un(), Aa(), Ca(), Ce(), ci(), Da(), De(), di() (+22 more)

### Community 94 - "Community 94"
Cohesion: 0.11
Nodes (34): addItem(), At(), Bl(), bs(), bt(), concat(), findField(), fp() (+26 more)

### Community 96 - "Community 96"
Cohesion: 0.09
Nodes (20): columns, Input, InputTextProps, InputRoot, BottomArrow, Default, LongText, meta (+12 more)

### Community 97 - "Community 97"
Cohesion: 0.06
Nodes (31): dependencies, @grpc/grpc-js, @grpc/proto-loader, jsonwebtoken, description, devDependencies, rimraf, @types/jsonwebtoken (+23 more)

### Community 98 - "Community 98"
Cohesion: 0.08
Nodes (32): zl(), aa(), dm(), et(), fm(), getAllClientExtensions(), getAllComputedFields(), getAllModelExtensions() (+24 more)

### Community 99 - "Community 99"
Cohesion: 0.12
Nodes (32): addMarginSymbol(), afterNextNewline(), compare(), copy(), de(), ee(), fa(), getCurrentLineLength() (+24 more)

### Community 102 - "Community 102"
Cohesion: 0.12
Nodes (16): createAclsTestCases(), createAgentsTestCases(), createApiKeysTestCases(), createApplicationsTestCases(), createCallsTestCases(), createCredentialsTestCases(), createDomainsTestCases(), createNumbersTestCases() (+8 more)

### Community 103 - "Community 103"
Cohesion: 0.10
Nodes (16): Create, Update, TWILIO_PSTN_URI_BASE, assignTwilioNumberToTrunk(), createOrUpdateApplication(), createTwilioTrunk(), getFonosterNumberByTelUrl(), getFonosterTrunkByInboundUri() (+8 more)

### Community 104 - "Community 104"
Cohesion: 0.07
Nodes (30): compilerOptions, baseUrl, esModuleInterop, jsx, lib, module, moduleResolution, noEmit (+22 more)

### Community 105 - "Community 105"
Cohesion: 0.10
Nodes (31): Cr(), Zc(), Cn(), addMarginSymbol(), afterNextNewline(), ap(), getCurrentLineLength(), getPrintWidth() (+23 more)

### Community 106 - "Community 106"
Cohesion: 0.13
Nodes (31): ze(), addMarginSymbol(), afterNextNewline(), compare(), copy(), getCurrentLineLength(), getPrintWidth(), indent() (+23 more)

### Community 107 - "Community 107"
Cohesion: 0.20
Nodes (6): Calls, statusStreamGenerator(), makeRpcRequest(), FonosterClient, dialStatusToString(), PartialApplicationInput

### Community 109 - "Community 109"
Cohesion: 0.10
Nodes (19): D(), getDataAttributes(), H(), I(), M(), N(), O(), off() (+11 more)

### Community 110 - "Community 110"
Cohesion: 0.07
Nodes (29): fluent-logger, author, bugs, url, dependencies, fluent-logger, winston, description (+21 more)

### Community 111 - "Community 111"
Cohesion: 0.14
Nodes (29): bc(), be(), dc(), fc(), findField(), gc(), getArgumentName(), getArgumentPath() (+21 more)

### Community 114 - "Community 114"
Cohesion: 0.15
Nodes (15): PlayDtmfRequest, StreamEvent, sandbox, sandbox, sandbox, sandbox, getVoiceObject(), voiceRequest (+7 more)

### Community 115 - "Community 115"
Cohesion: 0.07
Nodes (6): Prisma__ApiKeyClient, Prisma__UserClient, Prisma__VerificationCodeClient, Prisma__WorkspaceClient, Prisma__WorkspaceMemberClient, PrismaPromise

### Community 116 - "Community 116"
Cohesion: 0.16
Nodes (3): ct, it(), ot()

### Community 117 - "Community 117"
Cohesion: 0.11
Nodes (28): addMarginSymbol(), afterNextNewline(), bp(), getCurrentLineLength(), getPrintWidth(), handleRequestError(), indent(), is() (+20 more)

### Community 118 - "Community 118"
Cohesion: 0.12
Nodes (27): addMarginSymbol(), afterNextNewline(), Fr(), getCurrentLineLength(), getPrintWidth(), indent(), indentedCurrentLine(), io() (+19 more)

### Community 119 - "Community 119"
Cohesion: 0.15
Nodes (28): ac(), au(), be(), bn(), cc(), findField(), Ft(), getArgumentName() (+20 more)

### Community 120 - "Community 120"
Cohesion: 0.10
Nodes (28): C(), Ae(), Ai(), alloc(), allocUnsafe(), allocUnsafeSlow(), byteLength(), construct() (+20 more)

### Community 121 - "Community 121"
Cohesion: 0.10
Nodes (28): bl(), cr(), De(), Dr(), e(), Ee(), getOrCreate(), Gr() (+20 more)

### Community 122 - "Community 122"
Cohesion: 0.11
Nodes (5): AbstractClient, isJwtExpired(), TokenRefresherNode, ClientReadableStream, TokenRefresherWeb

### Community 123 - "Community 123"
Cohesion: 0.07
Nodes (27): compilerOptions, esModuleInterop, forceConsistentCasingInFileNames, lib, module, moduleResolution, noImplicitAny, outDir (+19 more)

### Community 124 - "Community 124"
Cohesion: 0.07
Nodes (27): ansis, cli-table3, cliui, figlet, @inquirer/prompts, dependencies, ansis, cli-table3 (+19 more)

### Community 125 - "Community 125"
Cohesion: 0.07
Nodes (27): ari-client, @fonoster/authz, @google-cloud/text-to-speech, microsoft-cognitiveservices-speech-sdk, dependencies, ari-client, @fonoster/authz, @fonoster/identity (+19 more)

### Community 126 - "Community 126"
Cohesion: 0.07
Nodes (27): @esm-bundle/chai, devDependencies, @esm-bundle/chai, rollup, @rollup/plugin-commonjs, @rollup/plugin-node-resolve, @rollup/plugin-replace, @rollup/plugin-terser (+19 more)

### Community 127 - "Community 127"
Cohesion: 0.12
Nodes (17): APP_REF_HEADER, AUTOPILOT_INTERNAL_ADDRESS, GRPC_NOT_SERVING_STATUS, GRPC_SERVING_STATUS, ROUTR_DEFAULT_PEER_AOR, WELCOME_DEMO_SPECIAL_LOCAL_ADDRESS, statusMap, sandbox (+9 more)

### Community 128 - "Community 128"
Cohesion: 0.07
Nodes (25): About Identity, Key Features, Specification, identity Specification, Purpose, Requirement: Id, access, and refresh tokens, Requirement: Refresh-token rotation policy, Requirement: Role-based access control (+17 more)

### Community 129 - "Community 129"
Cohesion: 0.10
Nodes (27): A(), bn(), ce(), enabled(), get(), has(), hn(), hs() (+19 more)

### Community 130 - "Community 130"
Cohesion: 0.13
Nodes (27): _a(), Ci(), Ct(), ge(), getArgumentName(), getArgumentPath(), _getName(), getSelectionPath() (+19 more)

### Community 131 - "Community 131"
Cohesion: 0.20
Nodes (14): AclsClient, AgentsClient, ApplicationsClient, CallsClient, ClientFunction, DataResponse, ServerStream, ServerStreamFunction (+6 more)

### Community 132 - "Community 132"
Cohesion: 0.24
Nodes (25): a(), b(), c(), d(), e(), f(), g(), h() (+17 more)

### Community 133 - "Community 133"
Cohesion: 0.09
Nodes (26): addItem(), as(), bp(), fs(), ge(), getAllClientExtensions(), getAllComputedFields(), getAllModelExtensions() (+18 more)

### Community 136 - "Community 136"
Cohesion: 0.08
Nodes (25): author, bugs, url, description, directories, src, test, files (+17 more)

### Community 137 - "Community 137"
Cohesion: 0.10
Nodes (3): xt, Ct(), Tt()

### Community 138 - "Community 138"
Cohesion: 0.19
Nodes (19): SidebarContext, SidebarContextProps, SidebarProvider(), SidebarProviderProps, useSidebar(), SidebarItem, NavItem(), NavItemProps (+11 more)

### Community 139 - "Community 139"
Cohesion: 0.10
Nodes (24): an(), cl(), fl(), get(), getCurrentBinaryTarget(), getURLAndAPIKey(), Gt(), Ii() (+16 more)

### Community 140 - "Community 140"
Cohesion: 0.10
Nodes (25): ba(), Ca(), Gi(), ha(), handleAndLogRequestError(), handleRequestError(), ht(), If() (+17 more)

### Community 141 - "Community 141"
Cohesion: 0.14
Nodes (3): e(), Yt, oe()

### Community 142 - "Community 142"
Cohesion: 0.08
Nodes (23): author, bin, fonoster, bugs, url, description, directories, src (+15 more)

### Community 143 - "Community 143"
Cohesion: 0.12
Nodes (24): convertProtocolErrorsToClientError(), cp(), getURLAndAPIKey(), handleError(), Hr(), ip(), je(), json() (+16 more)

### Community 144 - "Community 144"
Cohesion: 0.11
Nodes (22): ai(), ar(), Ca(), d(), di(), Et(), g(), h() (+14 more)

### Community 145 - "Community 145"
Cohesion: 0.11
Nodes (24): Cd(), destroy(), digest(), digestInto(), ec(), ed(), finish(), Id() (+16 more)

### Community 146 - "Community 146"
Cohesion: 0.11
Nodes (24): concat(), Ei(), eo(), fe(), getAllClientExtensions(), getAllComputedFields(), getAllModelExtensions(), getAllQueryCallbacks() (+16 more)

### Community 147 - "Community 147"
Cohesion: 0.09
Nodes (24): es(), getAllClientExtensions(), getAllModelExtensions(), gu(), includes(), indexOf(), isEmpty(), isPreviewFeatureOn() (+16 more)

### Community 148 - "Community 148"
Cohesion: 0.18
Nodes (14): logger, main(), createCallPrompt(), registerPrompts(), CreateCallBatchSchema, CreateCallSchema, ListApplicationsSchema, ListNumbersSchema (+6 more)

### Community 152 - "Community 152"
Cohesion: 0.08
Nodes (23): author, bugs, url, description, directories, src, test, engines (+15 more)

### Community 154 - "Community 154"
Cohesion: 0.10
Nodes (23): addItem(), constructor(), e(), ec(), getAllBatchQueryCallbacks(), getGlobalOmit(), _getName(), _getNamespace() (+15 more)

### Community 155 - "Community 155"
Cohesion: 0.09
Nodes (23): Vl(), ao(), Bo(), Ce(), constructor(), Fe(), getAllBatchQueryCallbacks(), getAllQueryCallbacks() (+15 more)

### Community 156 - "Community 156"
Cohesion: 0.10
Nodes (18): AnyNull, Args, Config, Constructor, DbNull, Exact, GetRuntimeOutput, Instance (+10 more)

### Community 157 - "Community 157"
Cohesion: 0.09
Nodes (22): Ip(), Op(), Ct(), ds(), ep(), getLocation(), gi(), gp() (+14 more)

### Community 158 - "Community 158"
Cohesion: 0.13
Nodes (12): jsonToObject(), objectToJson(), MappingTuple, getEnumKey(), getEnumValue(), isMapping(), enumMapping, ExampleEnum (+4 more)

### Community 160 - "Community 160"
Cohesion: 0.15
Nodes (18): AddWorkspaceCard(), meta, RegularCard, Story, WorkspaceCardProps, StyledAddIconContainer, StyledBottomContainer, StyledCardContentContainer (+10 more)

### Community 161 - "Community 161"
Cohesion: 0.10
Nodes (22): Aa(), ap(), dispatchBatches(), En(), fo(), getOrCreate(), gr(), handleAndLogRequestError() (+14 more)

### Community 162 - "Community 162"
Cohesion: 0.10
Nodes (22): ou(), cf(), Df(), getGlobalOmit(), gf(), includes(), indexOf(), isEncoding() (+14 more)

### Community 163 - "Community 163"
Cohesion: 0.11
Nodes (22): alloc(), allocUnsafe(), allocUnsafeSlow(), Bo(), br(), Cm(), construct(), Dt() (+14 more)

### Community 164 - "Community 164"
Cohesion: 0.12
Nodes (16): fluent, FluentTransport, LOGS_DRIVER_HOST, LOGS_DRIVER_PORT, LOGS_FORMAT, LOGS_LEVEL, LOGS_OPT_TAG_PREFIX, LOGS_TRANSPORT (+8 more)

### Community 180 - "Community 180"
Cohesion: 0.09
Nodes (21): **/bin/*, es2021, es6, scripthost, compilerOptions, composite, declaration, downlevelIteration (+13 more)

### Community 181 - "Community 181"
Cohesion: 0.10
Nodes (20): author, bin, fonoster, bugs, url, description, directories, src (+12 more)

### Community 182 - "Community 182"
Cohesion: 0.10
Nodes (21): dependencies, express, @fonoster/common, @fonoster/types, @grpc/grpc-js, jsonwebtoken, jwt-decode, nanoid (+13 more)

### Community 183 - "Community 183"
Cohesion: 0.14
Nodes (21): am(), ba(), Fr(), getAllClientExtensions(), getAllComputedFields(), getAllModelExtensions(), getComputedFields(), he() (+13 more)

### Community 184 - "Community 184"
Cohesion: 0.16
Nodes (21): at(), br(), co(), getArgumentName(), getArgumentPath(), _getName(), getSelectionPath(), Ke() (+13 more)

### Community 186 - "Community 186"
Cohesion: 0.14
Nodes (9): StartStreamRequest, StreamAudioFormat, StreamDirection, StreamMessageType, StreamPayload, StartStream, StopStream, Stream (+1 more)

### Community 187 - "Community 187"
Cohesion: 0.13
Nodes (14): LogoLarge, LogoProps, LogoMark, LogoSmall, LargeLogo, meta, MicroLogo, Story (+6 more)

### Community 188 - "Community 188"
Cohesion: 0.11
Nodes (15): config, fs, path, Prisma, PrismaClient, {
  PrismaClientKnownRequestError,
  PrismaClientUnknownRequestError,
  PrismaClientRustPanicError,
  PrismaClientInitializationError,
  PrismaClientValidationError,
  getPrismaClient,
  sqltag,
  empty,
  join,
  raw,
  skip,
  Decimal,
  Debug,
  objectEnumValues,
  makeStrictEnum,
  Extensions,
  warnOnce,
  defineDmmfProperty,
  Public,
  getRuntime,
  createParam,
}, { warnEnvConflicts }, empty() (+7 more)

### Community 189 - "Community 189"
Cohesion: 0.12
Nodes (13): Bi(), Bt(), handleAndLogRequestError(), handleRequestError(), He(), Ht(), _l(), renderAllMessages() (+5 more)

### Community 190 - "Community 190"
Cohesion: 0.22
Nodes (3): Xe(), Ge, qe()

### Community 191 - "Community 191"
Cohesion: 0.10
Nodes (5): AsObject, ContactType, DeleteUserRequest, InviteUserToWorkspaceResponse, Oauth2Provider

### Community 194 - "Community 194"
Cohesion: 0.11
Nodes (19): @aws-sdk/client-s3, @dmitryrechkin/json-schema-to-zod, @fonoster/voice, @langchain/anthropic, @langchain/groq, @langchain/ollama, dependencies, @aws-sdk/client-s3 (+11 more)

### Community 195 - "Community 195"
Cohesion: 0.18
Nodes (7): OPENAI_API_KEY, AbstractKnowledgeBase, logger, FilesKnowledgeBase, S3KnowledgeBase, KnowledgeBase, S3KnowledgeBaseParams

### Community 196 - "Community 196"
Cohesion: 0.17
Nodes (8): b(), be(), ee(), k, l(), q(), W(), y()

### Community 197 - "Community 197"
Cohesion: 0.14
Nodes (18): $c(), d(), Ee(), g(), getAllClientExtensions(), getAllModelExtensions(), gs(), Gt() (+10 more)

### Community 198 - "Community 198"
Cohesion: 0.11
Nodes (18): author, browser, bugs, url, description, files, gitHead, homepage (+10 more)

### Community 200 - "Community 200"
Cohesion: 0.11
Nodes (16): Example, Installation, new VoiceResponse(request, voice), VoiceResponse ⇐ <code>Verb</code>, voiceResponse.dial(destination, options) ⇒ <code>Promise.&lt;DialStatusStream&gt;</code>, voiceResponse.gather(options), voiceResponse.mute(options), voiceResponse.on(event, listener) (+8 more)

### Community 201 - "Community 201"
Cohesion: 0.11
Nodes (17): createApiKeyRequestSchema, createUserRequestSchema, createUserWithOauth2CodeRequestSchema, createWorkspaceRequestSchema, exchangeApiKeysRequestSchema, exchangeCredentialsRequestSchema, exchangeOauth2RequestSchema, exchangeRefreshTokenRequestSchema (+9 more)

### Community 207 - "Community 207"
Cohesion: 0.22
Nodes (4): _t(), mt, dt(), ht()

### Community 208 - "Community 208"
Cohesion: 0.17
Nodes (18): convertProtocolErrorsToClientError(), ep(), getURLAndAPIKey(), handleError(), je(), json(), metrics(), Ms() (+10 more)

### Community 209 - "Community 209"
Cohesion: 0.12
Nodes (18): alloc(), allocUnsafe(), allocUnsafeSlow(), be(), byteLength(), construct(), fi(), fill() (+10 more)

### Community 210 - "Community 210"
Cohesion: 0.19
Nodes (7): Applications, buildStructOverride(), buildStructOverrideReverse(), EvaluateIntelligenceEvent, EvaluateIntelligenceRequest, ListApplicationsRequest, ListApplicationsResponse

### Community 218 - "Community 218"
Cohesion: 0.12
Nodes (17): handlebars, dependencies, dotenv, @grpc/grpc-js, handlebars, jsonwebtoken, jwt-decode, nodemailer (+9 more)

### Community 219 - "Community 219"
Cohesion: 0.12
Nodes (16): author, bugs, url, description, gitHead, homepage, license, main (+8 more)

### Community 220 - "Community 220"
Cohesion: 0.13
Nodes (12): AssistantRawConfig, AUTOPILOT_YAML, findApiKey(), injectCredentials(), Integration, INTEGRATIONS_JSON, LanguageModelConfig, PROVIDER_PRODUCT_REF (+4 more)

### Community 221 - "Community 221"
Cohesion: 0.26
Nodes (11): useDataTable(), DataTableColumnHeader(), DataTableHead(), DataTableRoot(), DataTableContainerElement, DataTableRootElement, DataTableToolbarElement, TableRoot (+3 more)

### Community 222 - "Community 222"
Cohesion: 0.14
Nodes (17): bc(), Bt(), _c(), dr(), getAllClientExtensions(), I(), jn(), jr() (+9 more)

### Community 223 - "Community 223"
Cohesion: 0.12
Nodes (7): AdapterInfo, DriverAdapterFactory, Queryable, SqlDriverAdapter, SqlDriverAdapterFactory, SqlQueryable, $transaction()

### Community 225 - "Community 225"
Cohesion: 0.12
Nodes (16): ADDED Requirements, MODIFIED Requirements, Requirement: Id, access, and refresh tokens, Requirement: Realm isolation, Requirement: Realm resolution on operations, Requirement: Token verification via public key, Requirement: Users, workspaces, and resource ownership, Scenario: A consumer verifies a realm's token (+8 more)

### Community 226 - "Community 226"
Cohesion: 0.12
Nodes (15): author, bugs, url, description, files, gitHead, homepage, dist (+7 more)

### Community 227 - "Community 227"
Cohesion: 0.15
Nodes (13): MUST_BE_A_SINGLE_CHARACTER, MUST_BE_A_STRING, POSITIVE_INTEGER_MESSAGE, VALID_DATE, VALID_DTMF, VALID_LANGUAGE_CODE, VALID_URL, VALID_UUID (+5 more)

### Community 228 - "Community 228"
Cohesion: 0.24
Nodes (12): DATA_TABLE_FEATURES, DataTableContext, DataTableProvider(), BaseDataTable, CursorPagination, DataTable, DataTableContextProps, DataTableContextProviderProps (+4 more)

### Community 229 - "Community 229"
Cohesion: 0.12
Nodes (15): BodyLarge, BodyMedium, BodyMicro, BodySmall, BodySmallUnderline, DrawerLabel, DrawerTitle, HeadingLarge (+7 more)

### Community 230 - "Community 230"
Cohesion: 0.12
Nodes (15): author, bugs, url, description, files, gitHead, homepage, dist (+7 more)

### Community 231 - "Community 231"
Cohesion: 0.13
Nodes (16): bl(), Bo(), el(), eu(), getAllQueryCallbacks(), gl(), handleAndLogRequestError(), handleRequestError() (+8 more)

### Community 232 - "Community 232"
Cohesion: 0.24
Nodes (16): addMarginSymbol(), afterNextNewline(), getCurrentLineLength(), getPrintWidth(), indent(), newLine(), setColor(), underline() (+8 more)

### Community 233 - "Community 233"
Cohesion: 0.24
Nodes (16): addMarginSymbol(), afterNextNewline(), getCurrentLineLength(), getPrintWidth(), indent(), newLine(), setColor(), underline() (+8 more)

### Community 241 - "Community 241"
Cohesion: 0.12
Nodes (12): APIs, AudioSocket, audioSocket.onConnection(handler), AudioStream, audioStream.onClose(callback) ⇒ [<code>AudioStream</code>](#AudioStream), audioStream.onData(callback) ⇒ [<code>AudioStream</code>](#AudioStream), audioStream.onError(callback) ⇒ [<code>AudioStream</code>](#AudioStream), audioStream.play(filePath) ⇒ <code>Promise.&lt;void&gt;</code> (+4 more)

### Community 242 - "Community 242"
Cohesion: 0.23
Nodes (7): conversationSettingsSchema, EventsHookAllowedEvents, eventsHookSchema, languageModelConfigSchema, LanguageModelProvider, testCasesSchema, CallDirection

### Community 244 - "Community 244"
Cohesion: 0.13
Nodes (14): browser, imports, #main-entry-point, #wasm-engine-loader, main, default, name, sideEffects (+6 more)

### Community 245 - "Community 245"
Cohesion: 0.26
Nodes (15): B(), compare(), copy(), ns(), readIntBE(), readIntLE(), readUIntBE(), readUIntLE() (+7 more)

### Community 246 - "Community 246"
Cohesion: 0.13
Nodes (4): CreateExampleRequest, EmbeddedObject, ExampleEnum, sandbox

### Community 247 - "Community 247"
Cohesion: 0.14
Nodes (13): dependencies, @fonoster/sdk, description, engines, node, @fonoster/sdk, main, name (+5 more)

### Community 248 - "Community 248"
Cohesion: 0.14
Nodes (13): compilerOptions, outDir, rootDir, skipLibCheck, strictNullChecks, target, exclude, extends (+5 more)

### Community 249 - "Community 249"
Cohesion: 0.19
Nodes (6): Trunks, CreateTrunkRequest, ListTrunksRequest, ListTrunksResponse, Trunk, UpdateTrunkRequest

### Community 250 - "Community 250"
Cohesion: 0.18
Nodes (11): FilterField, DataTableToolbarFilters(), DropdownHeader, FilterOption, FilterSearchBySelector(), FilterSearchBySelectorProps, Trigger, UnifiedDropdown (+3 more)

### Community 251 - "Community 251"
Cohesion: 0.15
Nodes (13): App, columns, createPagination(), meta, mockData, NoFeatures, NoFilters, NoPagination (+5 more)

### Community 252 - "Community 252"
Cohesion: 0.18
Nodes (11): Checked, CheckedAndDisabled, DefaultValueFalse, DefaultValueTrue, meta, Story, Unchecked, UncheckedAndDisabled (+3 more)

### Community 254 - "Community 254"
Cohesion: 0.14
Nodes (13): compilerOptions, declaration, esModuleInterop, forceConsistentCasingInFileNames, module, moduleResolution, outDir, rootDir (+5 more)

### Community 255 - "Community 255"
Cohesion: 0.18
Nodes (13): Ba(), cr(), Fa(), h(), Ha(), I(), La(), pn() (+5 more)

### Community 260 - "Community 260"
Cohesion: 0.14
Nodes (13): compilerOptions, esModuleInterop, forceConsistentCasingInFileNames, module, noImplicitAny, outDir, rootDir, skipLibCheck (+5 more)

### Community 263 - "Community 263"
Cohesion: 0.15
Nodes (13): google-protobuf, grpc-web, dependencies, @fonoster/common, @fonoster/types, google-protobuf, @grpc/grpc-js, @grpc/proto-loader (+5 more)

### Community 264 - "Community 264"
Cohesion: 0.19
Nodes (10): baseApiObjectSchema, emptySchema, listRequestSchema, nameSchema, usernameSchema, createAclRequestSchema, ipOrCidr, updateAclRequestSchema (+2 more)

### Community 265 - "Community 265"
Cohesion: 0.15
Nodes (13): oclif, bin, commands, dirname, helpClass, plugins, topicSeparator, warn-if-update-available (+5 more)

### Community 266 - "Community 266"
Cohesion: 0.26
Nodes (3): equals(), Mt(), zt

### Community 267 - "Community 267"
Cohesion: 0.21
Nodes (6): Acls, Acl, CreateAclRequest, ListAclsRequest, ListAclsResponse, UpdateAclRequest

### Community 268 - "Community 268"
Cohesion: 0.21
Nodes (6): Agents, Agent, CreateAgentRequest, ListAgentsRequest, ListAgentsResponse, UpdateAgentRequest

### Community 270 - "Community 270"
Cohesion: 0.21
Nodes (6): Credentials, CreateCredentialsRequest, Credentials, ListCredentialsRequest, ListCredentialsResponse, UpdateCredentialsRequest

### Community 271 - "Community 271"
Cohesion: 0.15
Nodes (5): ApplicationType, AsObject, EvalError, EventCase, ExpectedTextType

### Community 272 - "Community 272"
Cohesion: 0.15
Nodes (12): ADDED Requirements, Requirement: Container image and accept-invite bridge, Requirement: Database provisioning entrypoint, Requirement: Externalized configuration with fail-fast validation, Requirement: Standalone Identity gRPC service, Scenario: Fresh database is provisioned, Scenario: Library import stays server-free, Scenario: Missing required configuration fails fast (+4 more)

### Community 273 - "Community 273"
Cohesion: 0.17
Nodes (11): compilerOptions, outDir, rootDir, target, exclude, extends, dist, node_modules (+3 more)

### Community 274 - "Community 274"
Cohesion: 0.35
Nodes (9): EvalIntelligence, buildEvalSummary(), EvalSummary, formatLine(), formatToolCalls(), printEvalError(), printScenarioHeader(), printScenarioSummary() (+1 more)

### Community 275 - "Community 275"
Cohesion: 0.26
Nodes (9): Workspace, SidebarProps, DEFAULT_WORKSPACE, FilterSearchBySelectorProps, WorkspaceOption, WorkspaceTrigger, WorkspaceUnifiedDropdown, WorkspaceSelector() (+1 more)

### Community 276 - "Community 276"
Cohesion: 0.26
Nodes (12): default, require, ./client, require, browser, default, edge-light, node (+4 more)

### Community 277 - "Community 277"
Cohesion: 0.26
Nodes (12): import, ./sql, browser, default, edge-light, node, types, worker (+4 more)

### Community 279 - "Community 279"
Cohesion: 0.17
Nodes (11): Build, docker, Environment Variables, Fonoster MCP Server, npx, Prompts, Setup, Testing with the MCP Inspector (+3 more)

### Community 280 - "Community 280"
Cohesion: 0.23
Nodes (5): Secrets, CreateSecretRequest, ListSecretsRequest, ListSecretsResponse, UpdateSecretRequest

### Community 310 - "Community 310"
Cohesion: 0.17
Nodes (11): Authors, Bugs and Feedback, Code Examples, Code memory (Graphify) — use this before grepping, Contributing, Features, Fonoster: The open-source alternative to Twilio, Getting Started (+3 more)

### Community 311 - "Community 311"
Cohesion: 0.18
Nodes (10): Asterisk PBX, Authors, Available Versions, Contributing, Environment Variables, Exposed ports, Installation, License (+2 more)

### Community 312 - "Community 312"
Cohesion: 0.18
Nodes (10): Check for context, Ending Discovery, Guardrails, Handling Different Entry Points, OpenSpec Awareness, The Stance, What You Don't Have To Do, What You Might Do (+2 more)

### Community 313 - "Community 313"
Cohesion: 0.18
Nodes (10): Ask for Help, Come to Meetings, Contributing Guide, DCO, Development Environment Setup, Find an Issue, Pull Request Checklist, Pull Request Lifecycle (+2 more)

### Community 314 - "Community 314"
Cohesion: 0.18
Nodes (11): devDependencies, @types/ari-client, @types/express, @types/jsonwebtoken, @types/uuid, @types/validator, @types/express, @types/jsonwebtoken (+3 more)

### Community 315 - "Community 315"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+2 more)

### Community 316 - "Community 316"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+2 more)

### Community 317 - "Community 317"
Cohesion: 0.22
Nodes (4): Args, BaseCommand, Flags, Login

### Community 318 - "Community 318"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+2 more)

### Community 319 - "Community 319"
Cohesion: 0.24
Nodes (8): RegionBadge(), DrawerExample, LandingPageExample, meta, Story, commonStyles, DrawerRegionBadge, LandingPageRegionBadge

### Community 320 - "Community 320"
Cohesion: 0.18
Nodes (10): Default, Disabled, Large, meta, Required, Story, WithCharacterLimit, WithError (+2 more)

### Community 321 - "Community 321"
Cohesion: 0.18
Nodes (11): exports, ./generator-build, ./index-browser, ./package.json, default, import, require, default (+3 more)

### Community 322 - "Community 322"
Cohesion: 0.22
Nodes (11): cu(), He(), k(), ke(), nu(), Ot(), pu(), qa() (+3 more)

### Community 324 - "Community 324"
Cohesion: 0.24
Nodes (11): ap(), Et(), ip(), is(), Jr(), lp(), ns(), op() (+3 more)

### Community 325 - "Community 325"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+2 more)

### Community 326 - "Community 326"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+2 more)

### Community 327 - "Community 327"
Cohesion: 0.18
Nodes (11): new Workspaces(client), Workspaces, workspaces.createWorkspace(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, workspaces.deleteWorkspace(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, workspaces.getWorkspace(ref) ⇒ <code>Promise.&lt;Acl&gt;</code>, workspaces.inviteUserToWorkspace(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, workspaces.listWorkspaceMembers(request) ⇒ <code>Promise.&lt;ListWorkspaceMembersResponse&gt;</code>, workspaces.listWorkspaces() ⇒ <code>Promise.&lt;ListWorkspacesResponse&gt;</code> (+3 more)

### Community 328 - "Community 328"
Cohesion: 0.18
Nodes (3): AsObject, DeleteAgentRequest, Privacy

### Community 329 - "Community 329"
Cohesion: 0.18
Nodes (3): AsObject, DeleteNumberResponse, IngressHandlerCase

### Community 330 - "Community 330"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+2 more)

### Community 331 - "Community 331"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+2 more)

### Community 332 - "Community 332"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+2 more)

### Community 333 - "Community 333"
Cohesion: 0.20
Nodes (9): Check for context, Ending Discovery, Guardrails, OpenSpec Awareness, The Stance, What You Don't Have To Do, What You Might Do, When a change exists (+1 more)

### Community 334 - "Community 334"
Cohesion: 0.29
Nodes (8): handleRpc(), makeClient(), PORT, rpcError(), rpcResult(), SERVER, TOOLS, toolSpecs()

### Community 335 - "Community 335"
Cohesion: 0.20
Nodes (9): Default, DifferentResourceTypes, LongValue, meta, SmallSize, Story, WithLeadingIcon, WithoutCopyIcon (+1 more)

### Community 336 - "Community 336"
Cohesion: 0.20
Nodes (9): Default, Disabled, meta, Required, sampleOptions, Story, WithError, WithHelperText (+1 more)

### Community 337 - "Community 337"
Cohesion: 0.22
Nodes (8): SIDEBAR_ITEMS, useSidebarItems(), withWorkspaceId(), Sidebar(), Default, meta, Story, workspaces

### Community 338 - "Community 338"
Cohesion: 0.24
Nodes (10): ce(), H(), An(), ce(), De(), gn(), He(), Pe() (+2 more)

### Community 340 - "Community 340"
Cohesion: 0.20
Nodes (9): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+1 more)

### Community 368 - "Community 368"
Cohesion: 0.20
Nodes (9): compilerOptions, outDir, rootDir, exclude, extends, dist, node_modules, test (+1 more)

### Community 369 - "Community 369"
Cohesion: 0.20
Nodes (9): ADDED Requirements, Requirement: Full Identity surface coverage from a single proto source, Requirement: Lightweight, endpoint-configurable client, Requirement: Per-request token forwarding, Scenario: A server forwards different callers' tokens, Scenario: Client targets an arbitrary endpoint, Scenario: Installing the client stays lightweight, Scenario: Stubs are generated from the canonical proto (+1 more)

### Community 370 - "Community 370"
Cohesion: 0.38
Nodes (7): c(), i(), l(), o(), r(), s(), u()

### Community 371 - "Community 371"
Cohesion: 0.22
Nodes (9): devDependencies, @oclif/test, @types/figlet, @types/js-yaml, @types/node, @types/node, @oclif/test, @types/figlet (+1 more)

### Community 372 - "Community 372"
Cohesion: 0.22
Nodes (8): author, description, homepage, license, name, private, type, version

### Community 373 - "Community 373"
Cohesion: 0.33
Nodes (9): a(), at(), dt(), gt(), lt(), st(), x(), xe() (+1 more)

### Community 374 - "Community 374"
Cohesion: 0.22
Nodes (9): as(), Ic(), ft(), ls(), rp(), ss(), us(), zr() (+1 more)

### Community 375 - "Community 375"
Cohesion: 0.33
Nodes (9): Fe(), Ge(), r(), i(), j(), le(), mn(), pn() (+1 more)

### Community 376 - "Community 376"
Cohesion: 0.22
Nodes (5): AnyNull, DbNull, JsonNull, NullTypesEnumValue, ObjectEnumValue

### Community 378 - "Community 378"
Cohesion: 0.22
Nodes (9): new Users(client), Users, users.createUser(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, users.createUserWithOauth2Code(request) ⇒ <code>Promise.&lt;ExchangeCredentialsResponse&gt;</code>, users.deleteUser(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, users.getUser(ref) ⇒ <code>Promise.&lt;Acl&gt;</code>, users.resetPassword(request) ⇒ <code>Promise.&lt;void&gt;</code>, users.sendResetPasswordCode(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code> (+1 more)

### Community 382 - "Community 382"
Cohesion: 0.22
Nodes (9): node-fetch, dependencies, dotenv, node-fetch, @prisma/client, xstate, dotenv, @prisma/client (+1 more)

### Community 383 - "Community 383"
Cohesion: 0.22
Nodes (9): optionalDependencies, @rollup/rollup-linux-arm64-gnu, @rollup/rollup-linux-arm64-musl, @rollup/rollup-linux-x64-gnu, @rollup/rollup-linux-x64-musl, @rollup/rollup-linux-arm64-gnu, @rollup/rollup-linux-arm64-musl, @rollup/rollup-linux-x64-gnu (+1 more)

### Community 384 - "Community 384"
Cohesion: 0.25
Nodes (7): Adding Knowledge Base, Adding Tools, Environment Variables, Example, Installation, Language Model Providers, What's Next

### Community 385 - "Community 385"
Cohesion: 0.25
Nodes (7): Adding Knowledge Base, Adding Tools, Environment Variables, Example, Installation, Language Model Providers, What's Next

### Community 386 - "Community 386"
Cohesion: 0.36
Nodes (6): SortOrder, DataTableColumnHeaderProps, SORT_OPTIONS, SortMenu(), SortMenuProps, TableCellRoot

### Community 387 - "Community 387"
Cohesion: 0.25
Nodes (7): Empty, Fair, Good, meta, Story, Strong, Weak

### Community 388 - "Community 388"
Cohesion: 0.25
Nodes (8): ./runtime/client, default, require, default, import, node, require, types

### Community 389 - "Community 389"
Cohesion: 0.32
Nodes (8): Cs(), Ds(), hp(), hu(), Is(), Qs(), st(), yp()

### Community 390 - "Community 390"
Cohesion: 0.25
Nodes (8): scripts, build, clean, generate:readme, postbuild, prebuild, start:demo, start:oauth2-demo

### Community 391 - "Community 391"
Cohesion: 0.25
Nodes (8): Applications, applications.createApplication(request) ⇒ <code>Promise.&lt;CreateAppResponse&gt;</code>, applications.deleteApplication(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, applications.evaluateIntelligence(request) ⇒ <code>Promise.&lt;ScenarioEvaluationReport&gt;</code>, applications.getApplication(ref) ⇒ <code>Promise.&lt;Application&gt;</code>, applications.listApplications(request) ⇒ <code>Promise.&lt;ListApplicationsResponse&gt;</code>, applications.updateApplication(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, new Applications(client)

### Community 451 - "Community 451"
Cohesion: 0.25
Nodes (7): ADDED Requirements, Requirement: Default realm for existing data, Requirement: Realm configuration, Scenario: A configured realm is usable, Scenario: An unknown realm is rejected, Scenario: Existing deployment keeps working, Scenario: Per-realm branding on outbound mail

### Community 452 - "Community 452"
Cohesion: 0.25
Nodes (7): Configuration source, Database ownership, Design notes, Reference implementation, Reuse, don't fork, What the standalone service is NOT, Why extend `mods/identity` instead of a new package

### Community 453 - "Community 453"
Cohesion: 0.25
Nodes (6): allJSFiles, fs, introFilePath, jsdoc2md, outputFile, path

### Community 454 - "Community 454"
Cohesion: 0.29
Nodes (7): scripts, build, build:storybook, dev, start, storybook, typecheck

### Community 455 - "Community 455"
Cohesion: 0.38
Nodes (4): DataRow(), DataTableBody(), DataTableBodyRowProps, lastUuidSegment()

### Community 456 - "Community 456"
Cohesion: 0.29
Nodes (7): devDependencies, prisma, @types/express, @types/jsonwebtoken, prisma, @types/express, @types/jsonwebtoken

### Community 457 - "Community 457"
Cohesion: 0.29
Nodes (3): ErrorWithBatchIndex, PrismaClientKnownRequestError, PrismaClientUnknownRequestError

### Community 458 - "Community 458"
Cohesion: 0.29
Nodes (5): empty(), config, Prisma, PrismaClient, {
  PrismaClientKnownRequestError,
  PrismaClientUnknownRequestError,
  PrismaClientRustPanicError,
  PrismaClientInitializationError,
  PrismaClientValidationError,
  getPrismaClient,
  sqltag,
  empty,
  join,
  raw,
  skip,
  Decimal,
  Debug,
  objectEnumValues,
  makeStrictEnum,
  Extensions,
  warnOnce,
  defineDmmfProperty,
  Public,
  getRuntime,
  createParam,
}

### Community 459 - "Community 459"
Cohesion: 0.29
Nodes (6): APIs, Creating a client object, Example, Importing the library, Installation, Login in and make requests

### Community 460 - "Community 460"
Cohesion: 0.29
Nodes (6): APIs, Creating a client object, Example, Importing the library, Installation, Login in and make requests

### Community 461 - "Community 461"
Cohesion: 0.29
Nodes (7): Acls, acls.createAcl(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, acls.deleteAcl(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, acls.getAcl(ref) ⇒ <code>Promise.&lt;Acl&gt;</code>, acls.listAcls(request) ⇒ <code>Promise.&lt;ListAclsResponse&gt;</code>, acls.updateAcl(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, new Acls(client)

### Community 462 - "Community 462"
Cohesion: 0.29
Nodes (7): Agents, agents.createAgent(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, agents.deleteAgent(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, agents.getAgent(ref) ⇒ <code>Promise.&lt;Acl&gt;</code>, agents.listAgents(request) ⇒ <code>Promise.&lt;ListAgentsResponse&gt;</code>, agents.updateAgent(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, new Agents(client)

### Community 463 - "Community 463"
Cohesion: 0.29
Nodes (7): Credentials, credentials.createCredentials(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, credentials.deleteCredentials(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, credentials.getCredentials(ref) ⇒ <code>Promise.&lt;Acl&gt;</code>, credentials.listCredentials(request) ⇒ <code>Promise.&lt;ListCredentialsResponse&gt;</code>, credentials.updateCredentials(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, new Credentials(client)

### Community 464 - "Community 464"
Cohesion: 0.29
Nodes (7): Domains, domains.createDomain(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, domains.deleteDomain(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, domains.getDomain(ref) ⇒ <code>Promise.&lt;Acl&gt;</code>, domains.listDomains(request) ⇒ <code>Promise.&lt;ListDomainsResponse&gt;</code>, domains.updateDomain(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, new Domains(client)

### Community 465 - "Community 465"
Cohesion: 0.29
Nodes (7): new Numbers(client), Numbers, numbers.createNumber(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, numbers.deleteNumber(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, numbers.getNumber(ref) ⇒ <code>Promise.&lt;Acl&gt;</code>, numbers.listNumbers(request) ⇒ <code>Promise.&lt;ListNumbersResponse&gt;</code>, numbers.updateNumber(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>

### Community 466 - "Community 466"
Cohesion: 0.29
Nodes (7): new Secrets(client), Secrets, secrets.createSecret(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, secrets.deleteSecret(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, secrets.getSecret(ref) ⇒ <code>Promise.&lt;Acl&gt;</code>, secrets.listSecrets(request) ⇒ <code>Promise.&lt;ListSecretsResponse&gt;</code>, secrets.updateSecret(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>

### Community 467 - "Community 467"
Cohesion: 0.29
Nodes (7): new Trunks(client), Trunks, trunks.createTrunk(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, trunks.deleteTrunk(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, trunks.getTrunk(ref) ⇒ <code>Promise.&lt;Acl&gt;</code>, trunks.listTrunks(request) ⇒ <code>Promise.&lt;ListTrunksResponse&gt;</code>, trunks.updateTrunk(request) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>

### Community 468 - "Community 468"
Cohesion: 0.29
Nodes (6): Auth model: per-request forwarding, Design notes, Proto is owned upstream, Reference implementation, Relationship to `@fonoster/sdk`, Why a separate package (and why that's consistent with Gap A)

### Community 469 - "Community 469"
Cohesion: 0.29
Nodes (6): Configuration shape, Decisions (locked), Design notes, Migration, Realm resolution, Relationship to the baseline

### Community 470 - "Community 470"
Cohesion: 0.29
Nodes (6): Capabilities, Impact, Modified Capabilities, New Capabilities, What Changes, Why

### Community 471 - "Community 471"
Cohesion: 0.29
Nodes (6): 1. Realm configuration (static file), 2. Realm-scoped, siloed data, 3. Per-realm keys and tokens, 4. Realm resolution, 5. Migration, 6. Verification

### Community 472 - "Community 472"
Cohesion: 0.29
Nodes (6): 1. Extract shared cross-cutting concerns from apiserver, 2. Make `mods/identity` runnable, 3. Configuration, 4. Database provisioning, 5. Packaging & release, 6. Verification

### Community 473 - "Community 473"
Cohesion: 0.29
Nodes (7): nodemonConfig, exec, ext, ignore, watch, workspaces, mods/*

### Community 474 - "Community 474"
Cohesion: 0.33
Nodes (6): scripts, build, clean, generate:readme, prebuild, start:dev

### Community 475 - "Community 475"
Cohesion: 0.40
Nodes (4): ROOT_DOMAIN, createDomainRequestSchema, domainSchema, updateDomainRequestSchema

### Community 477 - "Community 477"
Cohesion: 0.47
Nodes (3): Get, getOutboundUri(), Uri

### Community 478 - "Community 478"
Cohesion: 0.33
Nodes (5): Checked, Disabled, meta, NoLabel, Story

### Community 479 - "Community 479"
Cohesion: 0.33
Nodes (5): Default, Disabled, Loading, meta, Story

### Community 480 - "Community 480"
Cohesion: 0.33
Nodes (5): MultiValueRendererProps, SelectContainerProps, SelectInputProps, SelectOption, SelectProps

### Community 481 - "Community 481"
Cohesion: 0.33
Nodes (5): DisabledCard, meta, RegularCard, Story, WorkspaceCard()

### Community 482 - "Community 482"
Cohesion: 0.33
Nodes (6): scripts, build, clean, db:deploy, prebuild, start

### Community 483 - "Community 483"
Cohesion: 0.33
Nodes (5): config, Prisma, PrismaClient, {
  PrismaClientKnownRequestError,
  PrismaClientUnknownRequestError,
  PrismaClientRustPanicError,
  PrismaClientInitializationError,
  PrismaClientValidationError,
  getPrismaClient,
  sqltag,
  empty,
  join,
  raw,
  skip,
  Decimal,
  Debug,
  objectEnumValues,
  makeStrictEnum,
  Extensions,
  warnOnce,
  defineDmmfProperty,
  Public,
  getRuntime,
  createParam,
}, empty()

### Community 487 - "Community 487"
Cohesion: 0.33
Nodes (6): ApiKeys, apiKeys.createApiKey(request) ⇒ <code>Promise.&lt;CreateApiKeyResponse&gt;</code>, apiKeys.deleteApiKey(ref) ⇒ <code>Promise.&lt;BaseApiObject&gt;</code>, apiKeys.listApiKeys(request) ⇒ <code>Promise.&lt;ListApiKeysResponse&gt;</code>, apiKeys.regenerateApiKey(ref) ⇒ <code>Promise.&lt;CreateApiKeyResponse&gt;</code>, new ApiKeys(client)

### Community 488 - "Community 488"
Cohesion: 0.33
Nodes (5): AsObject, CallDirection, CallStatus, CallType, Status

### Community 489 - "Community 489"
Cohesion: 0.33
Nodes (5): Capabilities, Impact, New Capabilities, What Changes, Why

### Community 490 - "Community 490"
Cohesion: 0.33
Nodes (5): Capabilities, Impact, New Capabilities, What Changes, Why

### Community 491 - "Community 491"
Cohesion: 0.33
Nodes (4): destDir, fs, path, sourceDir

### Community 492 - "Community 492"
Cohesion: 0.40
Nodes (4): Agent instructions, Code memory (Graphify) — use this before grepping, Ops, Without MCP (primary for this tier)

### Community 493 - "Community 493"
Cohesion: 0.40
Nodes (4): Checklist:, Description, How Has This Been Tested?, Type of change

### Community 494 - "Community 494"
Cohesion: 0.40
Nodes (5): scripts, build, clean, prebuild, start:dev

### Community 495 - "Community 495"
Cohesion: 0.40
Nodes (5): scripts, build, clean, postbuild, prebuild

### Community 496 - "Community 496"
Cohesion: 0.40
Nodes (4): countryIsoCodeSchema, createNumberRequestSchema, sipUriSchema, updateNumberRequestSchema

### Community 497 - "Community 497"
Cohesion: 0.40
Nodes (4): createTrunkRequestSchema, hostOrIPSchema, updateTrunkRequestSchema, Transport

### Community 498 - "Community 498"
Cohesion: 0.40
Nodes (5): scripts, build, clean, generate:readme, prebuild

### Community 500 - "Community 500"
Cohesion: 0.40
Nodes (4): Client, @fonoster/sdk/dist/node/node.js, @fonoster/sdk/dist/web/index.esm.js, WebClient

### Community 501 - "Community 501"
Cohesion: 0.40
Nodes (3): {
  Decimal,
  objectEnumValues,
  makeStrictEnum,
  Public,
  getRuntime,
  skip
}, Prisma, PrismaClient

### Community 502 - "Community 502"
Cohesion: 0.40
Nodes (5): default, import, require, types, ./edge

### Community 503 - "Community 503"
Cohesion: 0.40
Nodes (5): ./extension, default, import, require, types

### Community 504 - "Community 504"
Cohesion: 0.40
Nodes (5): ./index, default, import, require, types

### Community 505 - "Community 505"
Cohesion: 0.40
Nodes (5): ./react-native, default, import, require, types

### Community 506 - "Community 506"
Cohesion: 0.40
Nodes (5): ./runtime/binary, default, import, require, types

### Community 507 - "Community 507"
Cohesion: 0.40
Nodes (5): ./runtime/edge, default, import, require, types

### Community 508 - "Community 508"
Cohesion: 0.40
Nodes (5): ./runtime/index-browser, default, import, require, types

### Community 509 - "Community 509"
Cohesion: 0.40
Nodes (5): ./runtime/library, default, import, require, types

### Community 510 - "Community 510"
Cohesion: 0.40
Nodes (5): ./runtime/react-native, default, import, require, types

### Community 511 - "Community 511"
Cohesion: 0.40
Nodes (5): ./runtime/wasm-compiler-edge, default, import, require, types

### Community 512 - "Community 512"
Cohesion: 0.40
Nodes (5): ./runtime/wasm-engine-edge, default, import, require, types

### Community 513 - "Community 513"
Cohesion: 0.40
Nodes (5): ./wasm, default, import, require, types

### Community 515 - "Community 515"
Cohesion: 0.40
Nodes (5): Calls, calls.createCall(request) ⇒ <code>Object</code>, calls.getCall(ref) ⇒ <code>Promise.&lt;CallDetailRecord&gt;</code>, calls.listCalls(request) ⇒ <code>Promise.&lt;ListCallsResponse&gt;</code>, new Calls(client)

### Community 516 - "Community 516"
Cohesion: 0.50
Nodes (3): @mui/material/styles, Palette, PaletteOptions

### Community 517 - "Community 517"
Cohesion: 0.50
Nodes (3): destDir, here, src

### Community 518 - "Community 518"
Cohesion: 0.50
Nodes (3): configPath, { database }, i

### Community 523 - "Community 523"
Cohesion: 0.50
Nodes (3): APIs, Example, Installation

### Community 524 - "Community 524"
Cohesion: 0.50
Nodes (3): 1. Proto and codegen, 2. Lightweight client package, 3. Verification

### Community 527 - "Community 527"
Cohesion: 0.67
Nodes (3): directories, src, test

### Community 528 - "Community 528"
Cohesion: 0.67
Nodes (3): files, dist, silero_vad.onnx

### Community 529 - "Community 529"
Cohesion: 0.67
Nodes (3): devDependencies, @types/nodemailer, @types/nodemailer

### Community 530 - "Community 530"
Cohesion: 0.67
Nodes (3): directories, src, test

### Community 531 - "Community 531"
Cohesion: 0.67
Nodes (3): repository, type, url

### Community 532 - "Community 532"
Cohesion: 0.67
Nodes (3): repository, type, url

### Community 538 - "Community 538"
Cohesion: 0.67
Nodes (3): bin, fonoster, fonoster-identity

### Community 539 - "Community 539"
Cohesion: 0.67
Nodes (3): directories, src, test

### Community 540 - "Community 540"
Cohesion: 0.67
Nodes (3): repository, type, url

### Community 546 - "Community 546"
Cohesion: 0.67
Nodes (3): directories, src, test

### Community 547 - "Community 547"
Cohesion: 0.67
Nodes (3): keywords, fonoster, sdk

### Community 548 - "Community 548"
Cohesion: 0.67
Nodes (3): repository, type, url

## Knowledge Gaps
- **3059 isolated node(s):** `check-preconditions.sh script`, `{ PrismaClient }`, `path`, `args`, `isDryRun` (+3054 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **307 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `A()` connect `Community 129` to `Community 99`, `Community 38`, `Community 42`, `Community 11`, `Community 139`, `Community 80`, `Community 51`, `Community 120`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `n()` connect `Community 119` to `Community 129`, `Community 130`, `Community 4`, `Community 8`, `Community 139`, `Community 11`, `Community 14`, `Community 144`, `Community 145`, `Community 146`, `Community 154`, `Community 155`, `Community 157`, `Community 161`, `Community 162`, `Community 38`, `Community 39`, `Community 40`, `Community 42`, `Community 43`, `Community 51`, `Community 54`, `Community 56`, `Community 57`, `Community 59`, `Community 61`, `Community 196`, `Community 197`, `Community 69`, `Community 72`, `Community 73`, `Community 76`, `Community 80`, `Community 83`, `Community 92`, `Community 93`, `Community 222`, `Community 375`, `Community 94`, `Community 98`, `Community 99`, `Community 231`, `Community 232`, `Community 105`, `Community 106`, `Community 233`, `Community 111`, `Community 373`, `Community 118`, `Community 117`, `Community 121`, `Community 255`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `kr()` connect `Community 73` to `Community 8`, `Community 11`, `Community 118`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Are the 108 inferred relationships involving `n()` (e.g. with `ee()` and `et()`) actually correct?**
  _`n()` has 108 INFERRED edges - model-reasoned connections that need verification._
- **What connects `check-preconditions.sh script`, `{ PrismaClient }`, `path` to the rest of the system?**
  _3059 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Community 0` be split into smaller, more focused modules?**
  _Cohesion score 0.0036101083032490976 - nodes in this community are weakly interconnected._
- **Should `Community 1` be split into smaller, more focused modules?**
  _Cohesion score 0.005479452054794521 - nodes in this community are weakly interconnected._